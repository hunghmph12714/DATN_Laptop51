...
(function ($, doc, win) {
    ...
    var templateGoogleMap = function (object, option) {
        ...
        this.build = function () {
            ...
            var mapBoxContent = $this.find(
                ".template-component-google-map-box-content"
            );
            var mapBoxContentId = helper.getRandomId(
                "template-component-google-map-box-content-"
            );
            ...
            if (mapButton.length === 1) {
                ...
                mapButton.on("click", function (e) {
                    ...
                    if (!$this.hasClass("template-state-open")) {
                        mapBox.animate(
                            { height: $option.dimension.height },
                            {
                                duration: 500,
                                complete: function () {
            ...
            var iframe =
                ...
                '" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyA2LLXoPbK_YPWDdpwgHaG3-PFYl6KiWhI&center=' +
                $option.coordinate.lat +
                "," +
                $option.coordinate.lng +
                "&q=" +
                $option.coordinate.lat +
                "," +
                $option.coordinate.lng +
                '&zoom=16" allowfullscreen="" frameborder="0"></iframe>';
            ...
            var googleMap = new google.maps.Map(
                document.getElementById(mapBoxContentId),
                $option.map
            );
